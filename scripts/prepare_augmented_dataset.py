#!/usr/bin/env python3
"""
Integration Guide: Using Augmented LIBERO Trajectories in VLA Training

This script demonstrates how to:
1. Load augmented trajectories
2. Combine with original trajectories
3. Create a data loader for VLA training
...
Example usage:
    python prepare_augmented_dataset.py \
        ...
        --output-jsonl training_data.jsonl
"""
...
def prepare_training_sample(trajectory: Dict, step_idx: int,  
                           ...
                           aug_idx: int = 0) -> Dict:
    """
    Prepare a single training sample (frame + action chunk + observation).
...
def create_mixed_dataset(original_hdf5: Path, augmented_dir: Path, 
    ...
    print(f"✓ Created training dataset with {len(samples)} samples")
...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare mixed dataset (original + augmented) for VLA training"
    )
    ...
    parser.add_argument("--output-jsonl", default="training_data_augmented.jsonl",
    ...
    print("\\n" + "=" * 80)
    print("PREPARE AUGMENTED DATASET FOR VLA TRAINING")
    print("=" * 80)
    ...
    print("=" * 80)
    print(f\"\"\"
1. Load training data in your VLA training script:
...
2. Create data loader:
   ...
   class AugmentedLiberoDataset(Dataset):
   ...
   dataset = AugmentedLiberoDataset(samples)
   loader = DataLoader(dataset, batch_size=32, shuffle=True)

3. Compare VLA training with/without augmentation:
   - Train on original only
   - Train on original + augmented
   - Measure performance difference

Expected improvements:
   - Better generalization to small perturbations
   - More robust action predictions
   - Improved zero-shot transfer to real robots
\"\"\")
