            ...
            with h5py.File(out_file, 'w') as f_out:
                ...
                for i, demo_key in enumerate(demo_keys):
                    ...
                    for _ in range(num_augmentations):
                        ...
                        # We MUST flip the images because LIBERO dataset wrapper natively flipped them
                        ...
                        obs_grp.create_dataset('agentview_rgb', data=np.concatenate([np.expand_dims(agentview_img, axis=0), orig_obs_grp['agentview_rgb'][t:]], axis=0))
                        obs_grp.create_dataset('eye_in_hand_rgb', data=np.concatenate([np.expand_dims(eye_in_hand_img, axis=0), orig_obs_grp['eye_in_hand_rgb'][t:]], axis=0))
